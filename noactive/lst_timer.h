...
#include <time.h>
#include <arpa/inet.h>
#include "../http/http_conn.h" // 加入定时器模块的方法：将任务类http_conn变成定时器类的内部类，然后将定时器类也变成http_conn的内部类互相包含

// 作用：定时检测不活跃的连接并及时把连接关闭掉
class http_conn; // 前向声明（当编译交叉包含的类的时候这部操作非常重要！！！！）
